...
constructor() {
  ...
  oneline)
    ...
    echo -n "$node_info$root_info$icon%n$reset$conda_info${virtualenv_info}$(git_status_info $git_status_style)$(git_prompt_info) %(?:$bold${green}➜ :$bold${red}➜ )${reset}${root_info}[$path_color%~$reset${line_color}] ─$(prompt_char)$reset "
    ;;
  esac
}
